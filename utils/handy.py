...
def generate_dipole(shape, z_prj=(0, 0, 1), vox=(1, 1, 1), shift=True, device=torch.device('cuda') if torch.cuda.is_available() else torch.device('cpu')):
...
def dipole_convolution_f(sus, dipole):
    ...
    return torch.real(fft.ifftn(dipole * fft.fftn(sus)))
